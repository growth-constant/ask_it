...
RSpec.describe AskIt::Survey do
  ...
  it 'does not create a survey with active flag true and empty questions collection' do
  ...
  it 'creates a survey with 3 sections' do
  ...
  it 'creates a survey with 2 questions' do
    num_questions = 2
  ...
  it 'does not create a survey with attempts_number lower than 0' do
    survey = build(:survey, attempts_number: -1)
    expect(survey).not_to be_valid
  end

  it 'does not save survey without all the needed fields' do
  ...
  it 'has the correct associations via "has_many_surveys"' do
